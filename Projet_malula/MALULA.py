...
class CardinalMalulaapp:
    ...
    def show_biographie(self):
        ...
        content.insert(END, "1. jeunesse\n", "titre")
        content.tag_config ("titre", font=("Arial", 14, "underline", "bold"))
...
En 1944, Joseph achève ses études de théologie. Après une année de stage au petit séminaire de Bokoro, il est ordonné prêtre le 9 juin 1946 au stade Reine-Astrid de Léopoldville en compagnie de son condisciple et ami, Eugène Moke.
        \n\n""")
                    #Resumer de sa vie
        content.insert(END, "2.Resumer de son parcour\n", "titre")
        content.tag_config ("titre", font=("Arial", 14, "underline", "bold"))
...
il meurt le 14 juin 1989, laissant un héritage spirituel et intellectuel important pour l'église et pour la société congolaise.       
        ...
        #Bouton retour
        back_button = Button(self.content_frame, text="Retour", font=("Arial", 12), command=lambda: self.return_menu(content,back_button))
        back_button.pack(pady=10)

    def return_menu(self, content, button):
        content.destroy()
        button.destroy()
...
\n\n""")
        content.insert(END, "2. l'imporrtance de l'éducation et l'église au services de la société \n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))
        content.insert(END, """Le cardinal Malula pensait que l'éducation est essentielle pour le développement de l'homme et de la société. selon lui, l'école doit former non seulement l'intelligence, mais aussi la conscience morale et spirituelle.
Pour Malula, l'église ne devait pas rester silencieusement face aux injustices. Elle devait défendre la vérité, la justice et la paix, même lorsque cela demandait du courage face au pouvoir politique.         
    ...
    def show_perspective(self):
        # Effacer le contenue actuel
        for widget in self.content_frame.winfo_children():
            widget.destroy()

        # afficher le contenue de la perspective
        content = Text(self.master, font=("Arial", 12), wrap=WORD)
        content.insert(END, """ La vision de Cardinal Malula s’inscrit dans un contexte de crise morale, économique et spirituelle.
Il a souligné :
« Les jeunes vivent dans une société où on dit tout et fait sans pudeur. »
...
> Problème majeur : l’éducation de qualité de la jeunesse, surtout à Kinshasa, ville jeune par excellence.
 ...
 """)
        content.insert(END, "1. Vision\n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))
        content.insert(END, """Le Cardinal Malula propose deux grandes lignes : les axes de formation et les principes pour l’éducation.
...
La vie spirituelle élève l’esprit et donne la priorité aux valeurs essentielles. Elle se manifeste par le sacrement et l’amour.
 """)
        ...
        content.insert(END, "2. Perspective\n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))

        content.insert(END, """Cardinal Malula propose quatre perspectives pour la jeunesse et la société :
...
4. Implication et éducation de l’humain authentique : Former des hommes authentiques, responsables et spirituellement et moralement solides.
 """)
    ...
    def show_principe(self):
        # Effacer le contenue actuel
        for widget in self.content_frame.winfo_children():
            widget.destroy()

        # afficher le contenue de la principe
        content = Text(self.master, font=("Arial", 12), wrap=WORD)
        content.insert(END, "1. Principes pour l'éducation\n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))

        content.insert(END, """ 1. Émergence : c’est le Refus de la médiocrité. La Volonté de progresser vers l’excellence et l’amour de la valeur.
...
\n\n """)
        ...
        content.insert(END, "2. Africanisation de l’Église et identité locale et Engagement social et justice\n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))
        content.insert(END, """Malula plaçait l’Église au service de la nation congolaise, prônant une insertion harmonieuse de la foi chrétienne dans le projet de société postcoloniale 
...
""")
    ...
    def show_ecole(self):
        ...
        # afficher le contenue de la perspective
        content = Text(self.master, font=("Arial", 12), wrap=WORD)
        content.insert(END, "1. Localisation et Infrastructure \n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))

        content.insert(END, """ Situé dans la commune de Limete, le complexe se trouve dans la concession de la maison des Sœurs de Sainte Thérèse de l’Enfant Jésus de Kinshasa. Il est composé de trois grandes directions :
...
\n\n""")
        ...
        content.insert(END, "2. Organisation et Ressources Humaines \n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))

        content.insert(END, """
Le complexe regroupe plus de 200 agents et suit un organigramme structuré comprenant :
...
\n\n
""")
        content.insert(END, "3. Philosophie et Objectifs Éducatifs \n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))
        content.insert(END, """Le Complexe Scolaire Cardinal Malula vise à :
...
\n\n""")
        ...
        content.insert(END, "4. Information sur le complexe cardinale Malula \n", "titre")
        content.tag_config("titre", font=("Arial", 14, "underline", "bold"))

        content.insert(END, """Composition
...
Et pour remercier l'école Complexe Scolaire Cardinal Malula ainsi que tous les enseignants qui m'ont poussé à donner le meilleur de moi-même, je tiens à exprimer ma profonde reconnaissance pour leur accompagnement, leur patience et leur engagement envers notre réussite. Grâce à leur dévouement et à leur savoir-faire, j'ai pu développer mes compétences et atteindre des objectifs que je n'aurais pas pu envisager seul.
 ...
 """)
        ...
        # Bouton retour
        back_button = Button(self.content_frame, text="Retour", font=("Arial", 12),
                             command=lambda: self.return_menu(content, back_button))
        back_button.pack(pady=10)


root = Tk()
app = CardinalMalulaapp(root)
root.geometry("600x700")
root.configure(background="#008080")
root.minsize(450, 360)
root.title("CARDINAL MALULA")
root.iconbitmap ("Logos1.ico")
